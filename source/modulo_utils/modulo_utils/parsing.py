...
def topic_validation_warning(name: str, type: str):
    ...
    """
    return (f"The parsed signal name for {type} '{name}' is empty. Provide a "
            "string with valid characters for the signal name ([a-z0-9_]).")


def parse_topic_name(topic_name: str) -> str:
    """
    Parse a string topic name from a user-provided input.
    This functions removes all characters different from a-z, 0-9, and _ from a string. Additionally, it removes all
    leading numbers and underscores, such that the resulting string starts with a letter a-z.

    :param topic_name: The input string
    :return: The sanitized string
    """
    sanitized_string = re.sub(r"[^a-z0-9_]", "", topic_name, flags=re.ASCII)
