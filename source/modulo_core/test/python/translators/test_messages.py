...
def test_accel(cart_state: sr.CartesianState, clock: Clock):
    ...
    assert_np_array_equal(
        read_xyz(
            message.linear),
        cart_state.get_linear_acceleration())
    assert_np_array_equal(
        read_xyz(
            message.angular),
        cart_state.get_angular_acceleration())
...
def test_pose(cart_state: sr.CartesianState, clock: Clock):
    ...
    assert_np_array_equal(
        read_xyz(
            message.position),
        cart_state.get_position())
    assert_np_array_equal(
        read_quaternion(
            message.orientation),
        cart_state.get_orientation_coefficients())
...
def test_twist(cart_state: sr.CartesianState, clock: Clock):
    ...
    assert_np_array_equal(
        read_xyz(
            message.linear),
        cart_state.get_linear_velocity())
    assert_np_array_equal(
        read_xyz(
            message.angular),
        cart_state.get_angular_velocity())
...
def test_joint_state(joint_state: sr.JointState):
    ...
    assert_np_array_equal(
        new_state.get_positions(),
        joint_state.get_positions())
    assert_np_array_equal(
        new_state.get_velocities(),
...
def test_encoded_state(cart_state: sr.CartesianState):
    ...
    modulo_writers.write_clproto_message(
        message, cart_state, clproto.MessageType.CARTESIAN_STATE_MESSAGE)
