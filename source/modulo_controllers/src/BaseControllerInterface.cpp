...
void BaseControllerInterface::pre_set_parameters_callback(std::vector<rclcpp::Parameter>& parameters) {
...
BaseControllerInterface::on_set_parameters_callback(const std::vector<rclcpp::Parameter>&) {
...
void BaseControllerInterface::add_tf_listener() {
  ...
  if (this->tf_buffer_ == nullptr || this->tf_listener_ == nullptr) {
    ...
    this->tf_listener_ = std::make_shared<tf2_ros::TransformListener>(*this->tf_buffer_, this->get_node());
  } else {
