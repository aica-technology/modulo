...
protected:
  ...
  void SetUp() override { component_ = std::make_shared<LifecycleComponentPublicInterface>(rclcpp::NodeOptions()); }
...
TEST_F(LifecycleComponentTest, AddRemoveOutput) {
  ...
  EXPECT_EQ(
      component_->outputs_.at("test")->get_message_pair()->get_type(),
